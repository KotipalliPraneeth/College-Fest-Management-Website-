...
session_start();
      ...
      //{
        ...
        //{
          ...
          $query = "SELECT users.id as id, users.user_name as name, participants.event_name as event
          FROM users
          JOIN participants ON users.id = participants.PID
          ";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        echo "<tr><td>" . $row["id"] . "</td><td>" . $row["name"] . "</td><td>" . $row["event"] . "</td></tr>";
    }
    echo "</table>";
} else {
    echo "0 result";
