...
session_start();
      ...
      //{
        ...
        //{
          ...
          $query = "SELECT accommodation.*, users.user_name as name
          FROM accommodation
          JOIN users ON accommodation.Id = users.id
          ";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        echo "<tr><td>" . $row["Id"] . "</td><td>" . $row["name"] . "</td><td>" . $row["days"] . "</td><td>". $row["starting_date"] ."</td><td>". $row["AC_room"] ."</td></tr>";
    }
    echo "</table>";
} else {
    echo "0 result";
